...
const CONFIG_PATH = path.join(process.env.XDG_CONFIG_HOME ||
    path.join(process.env.HOME || "/root", ".config"), "opencode", "provider-proxy.json");
...
const plugin = async () => {
    ...
    console.log("[opencode-provider-proxy] Active mappings:", providers.map((id) => `${id} → ${cfg[id]}`).join(", "));
    return {
        config: async (input) => {
